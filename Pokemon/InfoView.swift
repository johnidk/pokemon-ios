...
//  Pokemon
//
//  Created by João Gabriel Lavareda Ayres Barreto on 04/05/23.
//
...
class InfoView: UIView {
    ...
    let infoButton: UIButton = {
        ...
        button.addTarget(target, action: #selector(handleViewMoreInfo), for: .touchUpInside)
    ...
    @objc func handleViewMoreInfo() {
        guard let pokemon = self.pokemon else { return }
        delegate.self?.showMoreInfo(withPokemon: pokemon)
    ...
    func configureLabel(label: UILabel, title: String, details: String) {
        let attributedText = NSMutableAttributedString(
            attributedString: NSAttributedString(
                string: "\(title):  ",
                attributes: [NSAttributedString.Key.font : UIFont.boldSystemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.mainPink()]))
